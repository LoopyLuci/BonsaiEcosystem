...
### 1. Visualiser Proxy Layer (`crates/bonsai-mcp-server/src/visualiser.rs`)
...
### 2. MCP Server Refactor (`crates/bonsai-mcp-server/src/server.rs`)
...
### 3. CLI with Argument Parsing (`crates/bonsai-mcp-server/src/main.rs`)

**Full command-line interface** using `clap` crate:

```
Usage: bonsai-mcp-server [COMMAND]
...
cargo run -p bonsai-mcp-server -- headless --verbose --notify-on-error --popup-on-approval

# Quiet mode (production)
cargo run -p bonsai-mcp-server -- headless --quiet --log-path /var/log/bonsai.log
```
...
### 5. Updated Dependencies (`crates/bonsai-mcp-server/Cargo.toml`)
...
### 6. Module Integration (`crates/bonsai-mcp-server/src/lib.rs`)
...
```
1. User runs: cargo run -p bonsai-mcp-server -- visual --port 11426
...
```
1. User runs: cargo run -p bonsai-mcp-server -- headless --verbose --notify-on-error
...
```json
{
  "mcpServers": {
    "bonsai": {
      "command": "cargo",
      "args": ["run", "-p", "bonsai-mcp-server", "--", "headless", "--verbose", "--notify-on-error"],
...
| File | Purpose | Status |
|------|---------|--------|
| `crates/bonsai-mcp-server/src/visualiser.rs` | Core visualiser proxy | ✅ Complete |
| `crates/bonsai-mcp-server/src/main.rs` | CLI entry point | ✅ Complete |
| `crates/bonsai-mcp-server/src/server.rs` | Refactored MCP server | ✅ Complete |
| `crates/bonsai-mcp-server/src/lib.rs` | Updated module exports | ✅ Complete |
| `crates/bonsai-mcp-server/Cargo.toml` | Updated dependencies | ✅ Complete |
