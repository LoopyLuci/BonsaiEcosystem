...
use ui_utils::SwapBuffer;
use tokio::fs as tokio_fs;
use tracing::info;
use std::env;
use tokio::time::{timeout, Duration};
use runtime::{self, RuntimeManager};
use rand::Rng;
...
pub struct UIOrchestrator {
    ...
    runtimes: Mutex<HashMap<String, Box<dyn runtime::RuntimeController + Send + Sync>>> ,
...
mod tests {
    ...
    async fn open_temp_cas() -> cas::CasStore {
        let dir = tempdir().unwrap();
        let db_path = dir.path().join("cas.db");
        let blob_dir = dir.path().join("blobs");
        // leak so it lives for test duration
        std::mem::forget(dir);
        cas::CasStore::open(&db_path, &blob_dir).await.unwrap()
    }
