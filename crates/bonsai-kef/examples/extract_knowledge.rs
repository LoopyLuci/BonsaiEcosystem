...
async fn example_custom_extraction() -> Result<(), Box<dyn std::error::Error>> {
    use bonsai_kef::synthetic_generator::{SyntheticDataGenerator, SyntheticGeneratorConfig};
...
async fn example_quality_filtering() -> Result<(), Box<dyn std::error::Error>> {
    use bonsai_kef::quality_scorer::{QualityScorer, QualityScorerConfig};
...
fn example_pii_redaction() {
    use bonsai_kef::redaction::PiiRedactor;
