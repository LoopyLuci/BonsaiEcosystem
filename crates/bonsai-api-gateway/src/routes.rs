...
async fn pull_model(
    ...
    let info = bonsai_model_registry::pull::pull_model(name, &state.model_registry).await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))?;
...
async fn native_generate(
    ...
    let model_info = state.model_registry.get(&name, &version).await
        .ok_or_else(|| (StatusCode::NOT_FOUND, Json(serde_json::json!({"error": "Model not found"}))))?;
    let inference_req = bonsai_inference::InferenceRequest {
        model_name: name,
...
fn convert_request(req: &ChatCompletionRequest, model_info: &bonsai_model_registry::ModelInfo) -> bonsai_inference::InferenceRequest {
    let mut prompt = String::new();
    for msg in &req.messages {
        prompt.push_str(&format!("{}: {}\n", msg.role, msg.content));
    }
    bonsai_inference::InferenceRequest {
