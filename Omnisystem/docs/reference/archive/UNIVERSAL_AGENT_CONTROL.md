...
| File | Purpose |
|------|---------|
| `crates/mcp-server/src/uacs.rs` | Core UACS proxy with Visual/Headless modes |
| `crates/mcp-server/src/server.rs` | Refactored MCP server with UACS integration |
| `crates/mcp-server/src/main.rs` | CLI with `uacs visual` and `uacs headless` commands |
| `crates/mcp-server/src/lib.rs` | Module exports (includes uacs) |
| `crates/mcp-server/Cargo.toml` | Dependencies: clap, chrono, notify-rust |
...
Output: `target/release/mcp-server.exe` (Windows)
...
```powershell
# Change the port
cargo run -p mcp-server -- visual --port 11427
```
...
```bash
./target/release/mcp-server headless \
...
[Service]
Type=simple
ExecStart=/path/to/mcp-server headless --host 0.0.0.0 --port 8080 --quiet
Restart=always
RestartSec=10
...
COPY . /app
WORKDIR /app
RUN cargo build -p mcp-server --release
