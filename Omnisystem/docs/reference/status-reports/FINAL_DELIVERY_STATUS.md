...
Command: cargo build --package mcp-server --release
Binary Location: target/release/mcp-server.exe
```
...
**Result:** Clean minimal workspace ready for mcp-server build
...
2. Start server: `.\target\release\mcp-server.exe`
...
1. **[crates/mcp-server/src/bug_hunt_tools.rs](crates/mcp-server/src/bug_hunt_tools.rs)** - 7 handlers
2. **[crates/mcp-server/src/lint_tools.rs](crates/mcp-server/src/lint_tools.rs)** - 8 handlers
3. **[crates/mcp-server/src/tool_registry.rs](crates/mcp-server/src/tool_registry.rs)** - Dispatcher
...
1. **Verify binary:**
   ```powershell
   ls Z:\Projects\BonsaiWorkspace\target\release\mcp-server.exe
   ```
