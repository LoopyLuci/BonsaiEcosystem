# verify_H1.ps1 — Phase H1: Memory Tree Core
...
$modules = @("titan/omnibot/h1_memory.ti", "tests/test_h1.ti")
$regression = @("titan/omnicloak/c8_perf.ti", "titan/omnicloak/c7_verify.ti")
$failed = 0
Write-Host "`n=== Phase H1: Memory Tree Core ===" -ForegroundColor Cyan
...
Write-Host ""
if ($failed -eq 0) { Write-Host "RESULT: ALL $total VERIFIED — Phase H1 complete [score: 111]" -ForegroundColor Green; exit 0 }
