# verify_import_system.ps1 — cross-file import system: recursive merge, dedup, struct codegen
...
$modules = @(
    "tests/test_import_system.ti",
    "tests/test_struct_codegen.ti"
)

$failed = 0
Write-Host "`n=== Import System & Struct Codegen ===" -ForegroundColor Cyan
...
$total = $modules.Count
Write-Host ""
if ($failed -eq 0) { Write-Host "RESULT: ALL $total VERIFIED - Import system complete [score: 111]" -ForegroundColor Green; exit 0 }
