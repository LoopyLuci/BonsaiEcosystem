...
error: failed to select a version for `libsqlite3-sys`.
    ...
    ... which satisfies dependency `rusqlite = "^0.32"` of package `bonsai-universe`
...
- `bonsai-universe`, `bonsai-kdb` used `rusqlite 0.32`/`0.33` (requires `libsqlite3-sys 0.31.x`)
...
- `bonsai-universe` (rusqlite 0.32 → 0.33 → 0.37)
- `bonsai-kdb` (rusqlite 0.32 → 0.33 → 0.37)
- `bonsai-android-bridge` (transitive via bonsai-universe)
...
2. Unified all to `rusqlite 0.37` with `libsqlite3-sys 0.37`:
   ...
   - `bonsai-universe`: `0.32` → `0.33` → `0.37` (features: bundled)
...
2. Second attempt: After fixing 3 crates, build revealed bonsai-universe and bonsai-kdb also needed fixes
...
- [x] `bonsai-universe`: rusqlite `0.32` → `0.37`
...
| Second build attempt | 15 min | Failed: More rusqlite 0.32 in bonsai-universe/kdb | 2 |
...
**MCP Server Binary:** ✅ `target/release/bonsai-mcp-server.exe` (3.5 MB)  
**Build Command:** `cargo build --package bonsai-mcp-server --release`  
