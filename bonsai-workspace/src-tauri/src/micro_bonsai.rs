...
impl HardwareSnapshot {
    ...
    pub fn ram_pressure(&self) -> f32 {
        if self.ram_total_mb == 0 { return 0.0; }
...
impl MicroBonsai {
    pub fn new() -> Arc<Self> {
        ...
        Arc::new(Self { hw, history, _ticker: ticker })
    }
    ...
    fn score_model(
        ...
        match domain {
            TaskDomain::Code    => if id_lower.contains("code") || id_lower.contains("coder") { score += 1.5; }
            TaskDomain::Math    => if id_lower.contains("math") { score += 1.5; }
            TaskDomain::Creative => if id_lower.contains("creative") || id_lower.contains("story") { score += 1.0; }
            _ => {}
        }
        ...
        if hw.ram_free_mb < LOW_RAM_THRESHOLD || hw.vram_free_mb() < LOW_VRAM_THRESHOLD {
            if is_large_model(model_id) { score -= 3.0; }
            if is_small_model(model_id) { score += 1.0; }
        }

        // Power saving: heavily prefer small models
        if req.power_saving && is_large_model(model_id) { score -= 5.0; }

        // Historical quality and speed
        let domain_str = format!("{:?}", domain);
        let relevant: Vec<&PerfRecord> = hist.iter()
            .filter(|r| r.model_id == model_id && r.task_domain == domain_str)
            .collect();
        if !relevant.is_empty() {
            let avg_q = relevant.iter().map(|r| r.quality_score.max(0.0)).sum::<f32>()
                / relevant.len() as f32;
            let avg_tps = relevant.iter().map(|r| r.tokens_per_sec).sum::<f32>()
                / relevant.len() as f32;
        ...
        if id_lower.contains("bonsai") { score += 0.5; }
    ...
    pub async fn suggest_swarm(&self, req: &SelectionRequest) -> Option<String> {
        ...
        let can_swarm = hw.ram_free_mb >= LOW_RAM_THRESHOLD * 2
            || hw.vram_free_mb() >= LOW_VRAM_THRESHOLD * 2;

        if !can_swarm { return None; }
...
fn is_small_model(id: &str) -> bool {
    ...
    lower.contains("0.5b") || lower.contains("1b") || lower.contains("1.7b")
        || lower.contains("tiny") || lower.contains("mini")
}

fn is_large_model(id: &str) -> bool {
    let lower = id.to_lowercase();
    lower.contains("70b") || lower.contains("72b") || lower.contains("34b")
        || lower.contains("32b") || lower.contains("large")
}
...
pub async fn micro_select_model(
    ...
    let loaded: Vec<String> = status.slots.iter()
        .filter_map(|s| s.state.model_id().map(|m| m.to_string()))
        .collect();
    let available: Vec<String> = all_models.iter()
        .map(|m| m.id.clone())
        .collect();
