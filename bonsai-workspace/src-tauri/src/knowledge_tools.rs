...
impl Tool for ReasonTool {
    ...
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        let query = args["query"].as_str().ok_or("missing field: query")?;
    ...
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        let (status, details) = match &result {
            crate::belief_reviser::ConsistencyResult::Consistent => (
                "consistent",
                json!({ "message": "No contradictions found in knowledge graph." }),
            ),
            ...
            crate::belief_reviser::ConsistencyResult::Uncertain => (
                "uncertain",
                json!({ "message": "Cannot determine consistency from available knowledge." }),
            ),
        };
...
pub async fn register_knowledge_tools(
    ...
    registry
        .registry
        .register(Box::new(ReasonTool { engine }))
        .await;
    registry
        .registry
        .register(Box::new(KnowledgeQueryTool {
            knowledge: knowledge.clone(),
        }))
        .await;
    registry
        .registry
        .register(Box::new(FactCheckTool {
            knowledge,
            belief_reviser,
        }))
        .await;
