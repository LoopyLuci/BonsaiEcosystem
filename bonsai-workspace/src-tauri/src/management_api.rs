...
pub struct MgmtState {
    ...
    pub swarm_cancels: Arc<
        std::sync::Mutex<
            std::collections::HashMap<String, Vec<Arc<std::sync::atomic::AtomicBool>>>,
        >,
    >,
...
pub fn router(state: MgmtState) -> Router {
    Router::new()
        ...
        .route(
            "/api/v1/features",
            get(mgmt_get_features).post(mgmt_set_features),
        )
        ...
        .route(
            "/api/v1/telemetry/training/:id",
            get(mgmt_telemetry_training_run),
        )
        ...
        // self-play training
        .route(
            "/api/v1/training/self-play/start",
            post(mgmt_self_play_start),
        )
        .route("/api/v1/training/self-play/stop", post(mgmt_self_play_stop))
        .route(
            "/api/v1/training/self-play/status",
            get(mgmt_self_play_status),
        )
        // plugin host
        ...
        .route(
            "/api/v2/knowledge/entities",
            post(mgmt_knowledge_add_entity),
        )
...
async fn mgmt_list_models(State(s): State<MgmtState>, headers: HeaderMap) -> impl IntoResponse {
    auth!(s, headers);
...
async fn mgmt_queue_status(State(s): State<MgmtState>, headers: HeaderMap) -> impl IntoResponse {
    auth!(s, headers);
...
async fn mgmt_swarm_submit(
    ...
    let synthesis_ctx = worker_outputs
        .iter()
        .map(|w| {
            format!(
                "Worker {} ({}):\n{}",
...
async fn mgmt_swarm_metrics(State(s): State<MgmtState>, headers: HeaderMap) -> impl IntoResponse {
    auth!(s, headers);
    if !crate::features::FeatureFlags::is_enabled("swarm") {
        return err400("Swarm feature is disabled").into_response();
    }
...
async fn mgmt_list_agents(State(s): State<MgmtState>, headers: HeaderMap) -> impl IntoResponse {
    auth!(s, headers);
...
async fn mgmt_get_features(State(s): State<MgmtState>, headers: HeaderMap) -> impl IntoResponse {
    auth!(s, headers);
...
async fn mgmt_training_status(State(s): State<MgmtState>, headers: HeaderMap) -> impl IntoResponse {
    auth!(s, headers);
...
async fn mgmt_loop_stop(State(s): State<MgmtState>, headers: HeaderMap) -> impl IntoResponse {
    auth!(s, headers);
    s.training_loop.stop().await;
    Json(json!({ "ok": true, "status": "stopped" })).into_response()
}

async fn mgmt_loop_status(State(s): State<MgmtState>, headers: HeaderMap) -> impl IntoResponse {
    auth!(s, headers);
...
async fn mgmt_self_play_stop(State(s): State<MgmtState>, headers: HeaderMap) -> impl IntoResponse {
    auth!(s, headers);
...
async fn mgmt_plugin_load(
    ...
    match s
        .plugin_host
        .load(&body.id, std::path::Path::new(&body.path))
        .await
    {
...
async fn mgmt_plugin_list(State(s): State<MgmtState>, headers: HeaderMap) -> impl IntoResponse {
    auth!(s, headers);
...
async fn mgmt_tools_list(State(s): State<MgmtState>, headers: HeaderMap) -> impl IntoResponse {
    auth!(s, headers);
...
async fn mgmt_chess_new(
    ...
    Json(json!({ "game_id": id, "human_color": color, "ai_strength": strength, "view": view }))
        .into_response()
}
...
async fn mgmt_go_new(
    ...
    let size = if [9u8, 13, 19].contains(&size) {
        size
    } else {
        19
    };
    ...
    Json(json!({ "game_id": id, "board_size": size, "komi": komi, "human_color": color }))
        .into_response()
}
...
async fn mgmt_go_status(
    ...
    match sessions.get(&uid) {
        ...
        Some(session) => {
            ...
            Json(json!({ "game_id": id, "board_size": view.size, "result": view.result }))
                .into_response()
        }
    }
}
...
async fn mgmt_puzzle_daily(State(s): State<MgmtState>, headers: HeaderMap) -> impl IntoResponse {
    auth!(s, headers);
    match s.game_sessions.puzzles.daily() {
        None => (
            StatusCode::NOT_FOUND,
            Json(json!({"error": "no puzzle available"})),
        )
            .into_response(),
        Some(p) => Json(json!({
            "id": p.id,
...
async fn mgmt_puzzle_check(
    ...
    let result = s
        .game_sessions
        .puzzles
        .check_move(&req.puzzle_id, &req.uci_move);
    let body = match result {
        PuzzleCheckResult::Solved { explanation } => {
            json!({"status": "solved", "message": explanation})
        }
        PuzzleCheckResult::CorrectContinue { next_hint } => {
            json!({"status": "correct", "message": next_hint})
        }
...
async fn mgmt_tournament_list(State(s): State<MgmtState>, headers: HeaderMap) -> impl IntoResponse {
    auth!(s, headers);
...
async fn mgmt_belief_check(
    ...
    let (status, message) = match &result {
        ...
        crate::belief_reviser::ConsistencyResult::Contradicts {
            conflicting,
            max_conflict_confidence,
        } => (
            "contradicts",
            format!(
                "Contradicts {} beliefs (max confidence {:.2})",
                conflicting.len(),
                max_conflict_confidence
            ),
        ),
        crate::belief_reviser::ConsistencyResult::Uncertain => (
            "uncertain",
            "Cannot determine from available knowledge".to_string(),
        ),
    };
