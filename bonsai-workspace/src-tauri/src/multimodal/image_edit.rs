...
fn find_rapid_model() -> Option<PathBuf> {
    ...
    let base = dirs::home_dir().unwrap_or_default().join(".bonsai/models/image_edit/qwen-rapid");
...
fn find_multiangle_model() -> Option<PathBuf> {
    ...
    let base = dirs::home_dir().unwrap_or_default().join(".bonsai/models/image_edit/qwen-multiangle");
    if base.exists() { return Some(base); }
    None
}
...
fn find_worker() -> Option<PathBuf> {
    let candidates = [
        dirs::home_dir().unwrap_or_default().join(".bonsai/sidecars/image_edit_worker.py"),
        PathBuf::from("sidecars/image_edit_worker.py"),
    ];
    candidates.into_iter().find(|p| p.exists())
}

pub fn is_available() -> bool { find_rapid_model().is_some() && find_worker().is_some() }
...
async fn call_worker(payload: &Value) -> Result<Value, String> {
    ...
    let mut child = tokio::process::Command::new(&python)
        ...
        .kill_on_drop(true)
        .spawn().map_err(|e| format!("Failed to start image_edit_worker: {e}"))?;
    ...
    let out = tokio::time::timeout(Duration::from_secs(300), child.wait_with_output())
        .await.map_err(|_| "image_edit_worker timed out (300s)".to_string())?
...
async fn run_edit(image_path: &str, prompt: &str, save_path: Option<&str>) -> Result<ImageEditResult, String> {
    let model = find_rapid_model().ok_or("Qwen-Image-Edit model not found. Place model in ~/.bonsai/models/image_edit/qwen-rapid/")?;
    ...
    Ok(ImageEditResult {
        ...
        model:      model.file_name().and_then(|n| n.to_str()).unwrap_or("qwen-rapid").to_string(),
        prompt:     prompt.to_string(),
        elapsed_ms: t0.elapsed().as_millis() as u64,
    })
}

async fn run_generate_rapid(prompt: &str, save_path: Option<&str>) -> Result<ImageEditResult, String> {
...
async fn run_multiview(image_path: &str, angles: &[String]) -> Result<MultiviewResult, String> {
    ...
    let t0 = std::time::Instant::now();
    let default_angles = ["front", "side_left", "side_right", "back", "top", "isometric"];
    ...
    let result = call_worker(&payload).await?;
    let images_b64: Vec<String> = result["images_b64"].as_array()
        .map(|arr| arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect())
        .unwrap_or_default();
    let saved_paths: Vec<Option<String>> = result["saved_paths"].as_array()
    ...
    Ok(MultiviewResult {
        ...
        saved_paths,
        model: model.file_name().and_then(|n| n.to_str()).unwrap_or("qwen-multiangle").to_string(),
        elapsed_ms: t0.elapsed().as_millis() as u64,
    })
}
...
impl Tool for GenerateImageRapidTool {
    fn name(&self) -> &str { "generate_image_rapid" }
    fn description(&self) -> &str {
...
impl Tool for GenerateMultiviewTool {
    fn name(&self) -> &str { "generate_multiview" }
    fn description(&self) -> &str {
    ...
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        let angles: Vec<String> = args["angles"].as_array()
            .map(|arr| arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect())
            .unwrap_or_default();
...
#[tauri::command]
pub async fn edit_image_cmd(image_path: String, prompt: String, save_path: Option<String>) -> Result<ImageEditResult, String> {
...
#[tauri::command]
pub async fn generate_image_rapid_cmd(prompt: String, save_path: Option<String>) -> Result<ImageEditResult, String> {
    run_generate_rapid(&prompt, save_path.as_deref()).await
}

#[tauri::command]
pub async fn generate_multiview_cmd(image_path: String, angles: Option<Vec<String>>) -> Result<MultiviewResult, String> {
    if !std::path::Path::new(&image_path).exists() { return Err(format!("Image not found: {image_path}")); }
    run_multiview(&image_path, &angles.unwrap_or_default()).await
}

#[tauri::command]
pub async fn image_edit_available() -> Result<bool, String> { Ok(is_available()) }
