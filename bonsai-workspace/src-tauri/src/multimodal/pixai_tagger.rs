...
fn find_model() -> Option<PathBuf> {
    ...
    let candidates = [
        dirs::home_dir().unwrap_or_default().join(".bonsai/models/pixai/pixai-tagger-v0.9.onnx"),
...
fn find_worker() -> Option<PathBuf> {
    let candidates = [
        dirs::home_dir().unwrap_or_default().join(".bonsai/sidecars/pixai_worker.py"),
...
async fn run_tagger(image_path: &str, confidence: f32, max_tags: usize) -> Result<TagResult, String> {
    let model  = find_model().ok_or("PixAI model not found. Place pixai-tagger-v0.9.onnx in ~/.bonsai/models/pixai/")?;
    ...
    let mut child = tokio::process::Command::new(&python)
        ...
        .kill_on_drop(true)
        .spawn().map_err(|e| format!("Failed to start pixai_worker: {e}"))?;
    ...
    let out = tokio::time::timeout(Duration::from_secs(60), child.wait_with_output())
        .await.map_err(|_| "pixai_worker timed out".to_string())?
    ...
    let dominant = tags.first().map(|t| t.category.clone()).unwrap_or_else(|| "general".into());
    let count = tags.len();
    ...
    Ok(TagResult {
        ...
        tag_count: count,
        model: model.file_name().and_then(|n| n.to_str()).unwrap_or("pixai").to_string(),
...
impl Tool for TagImageTool {
    ...
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        if !std::path::Path::new(path).exists() { return Err(format!("Image not found: {path}")); }
        if !is_available() {
...
impl Tool for DescribeImageAnimeTool {
    fn name(&self) -> &str { "describe_image_anime" }
    fn description(&self) -> &str {
    ...
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        Ok(ToolResult::json(&json!({ "description": description, "tags": result.tags })))
    }
}
...
#[tauri::command]
pub async fn pixai_tag(image_path: String, confidence: Option<f32>, max_tags: Option<usize>) -> Result<TagResult, String> {
    if !std::path::Path::new(&image_path).exists() { return Err(format!("Image not found: {image_path}")); }
    run_tagger(&image_path, confidence.unwrap_or(0.35), max_tags.unwrap_or(50)).await
}

#[tauri::command]
pub async fn pixai_available() -> Result<bool, String> { Ok(is_available()) }
