...
```powershell
cargo run -p bonsai-mcp-server -- headless --verbose --notify-on-error
...
- `crates/bonsai-mcp-server/src/visualiser.rs` — Event proxy and modes
- `crates/bonsai-mcp-server/src/main.rs` — CLI with argument parsing
- `crates/bonsai-mcp-server/src/server.rs` — Refactored for visualiser
