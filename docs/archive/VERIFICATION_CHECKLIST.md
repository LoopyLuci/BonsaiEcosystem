...
- [ ] Build succeeds
- [ ] Binary created at `target/release/mcp-server.exe` (Windows) or `target/release/mcp-server` (Unix)
...
```bash
cargo run -p mcp-server -- visual --help
cargo run -p mcp-server -- headless --help
```
