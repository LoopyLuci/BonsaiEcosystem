...
The visualiser is now integrated into the `mcp-server` crate. To build it as a binary:
...
This creates an executable at `target/release/mcp-server` (or `.exe` on Windows).
...
```bash
cd Z:\Projects\BonsaiWorkspace
cargo run -p mcp-server -- headless --host 127.0.0.1 --port 11425 --verbose --notify-on-error
```
...
Or pass as flags:
```bash
cargo run -p mcp-server -- headless --log-path /tmp/agent.log
```
...
```bash
cargo run -p mcp-server -- headless --port 11425 &
cargo run -p mcp-server -- headless --port 11426 &
```
...
2. **Run the binary directly (faster than `cargo run`):**
   ```bash
   ./target/release/mcp-server headless --host 0.0.0.0 --port 8080 --quiet
   ```
...
3. **Use a systemd service or supervisor** to keep the process running:
   ...
   [Service]
   Type=simple
   ExecStart=/path/to/mcp-server headless --host 0.0.0.0 --port 8080
   Restart=always
   RestartSec=10
