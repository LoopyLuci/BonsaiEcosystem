...
```json
{
  "mcpServers": {
    "bonsai": {
      "command": "cargo",
      "args": ["run", "-p", "mcp-server", "--", "visual", "--port", "11426"]
    }
  }
}
```
...
- `crates/mcp-server/src/uacs.rs` — Core system
- `crates/mcp-server/src/main.rs` — CLI
- `crates/mcp-server/src/server.rs` — MCP integration
