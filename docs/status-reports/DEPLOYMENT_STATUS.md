...
- ✅ `crates/mcp-server/src/lint_commands.rs` – Handlers
- ✅ `crates/mcp-server/src/lint_integration.rs` – Integration layer
...
cargo test -p mcp-server lint --release
```
...
- Check MCP server is running: `cargo run -p mcp-server`
