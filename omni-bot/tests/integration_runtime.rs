...
async fn integration_runtime_start_stop() -> Result<(), Box<dyn std::error::Error>> {
    ...
    bot::session::migrate(&db).await?;
    let (tx, _rx) = mpsc::channel::<bot::admin_api::BroadcastRequest>(1);

    let admin_token = "itest-token".to_string();
    // start admin server on ephemeral port
    let mut handle =
        bot::admin_api::start(0, metrics, platform_states, db, tx, admin_token.clone())
            .await
