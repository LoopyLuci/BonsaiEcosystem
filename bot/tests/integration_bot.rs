...
use bot::platforms::{InboundMessage, MessagingPlatform, ShedNotice};
...
async fn make_router(buddy_url: String) -> Arc<bot::router::Router> {
    use bot::config::{BotConfig, CircuitBreakerConfig};
    use bot::{
        buddy_client::BuddyClient, dedup::DedupCache, health::CircuitBreaker, metrics::Metrics,
    ...
    bot::session::migrate(&db).await.unwrap();

    Arc::new(bot::router::Router::new(
        buddy,
        mgmt,
        dedup,
        db,
...
async fn circuit_breaker_opens_after_failures() {
    ...
    let db = Arc::new(Connection::open_in_memory().await.unwrap());
    bot::session::migrate(&db).await.unwrap();
    let router = Arc::new(bot::router::Router::new(
        buddy,
        mgmt,
        dedup,
        db,
